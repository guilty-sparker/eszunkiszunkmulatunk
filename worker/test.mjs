...
const first = await post({ guest: "p1", question: "q1", choice: "a" });
check("returns 200", first.status === 200, `got ${first.status}`);
check("echoes the choice", first.body?.choice === "a");
check("not flagged as already voted", first.body?.already === false);
check("counts p1", first.body?.counts?.q1?.a === 1);
check("zero-fills untouched choices", first.body?.counts?.q1?.d === 0);
...
check("returns the original choice", second.body?.choice === "a", `got ${second.body?.choice}`);
check("flagged as already voted", second.body?.already === true);
check("count for a unchanged", second.body?.counts?.q1?.a === 1);
...
check("a=1", third.body?.counts?.q1?.a === 1);
check("b=2", third.body?.counts?.q1?.b === 2, JSON.stringify(third.body?.counts?.q1));

console.log("validation");
check("guest id with illegal characters", (await post({ guest: "p1; drop", question: "q1", choice: "a" })).status === 400);
check("guest id too long", (await post({ guest: "p".repeat(65), question: "q1", choice: "a" })).status === 400);
check("empty guest id", (await post({ guest: "", question: "q1", choice: "a" })).status === 400);
check("unknown question", (await post({ guest: "p4", question: "q99", choice: "a" })).status === 400);
...
check("returns this guest's vote", mine.body?.votes?.q1 === "a");
...
check("includes every question", Object.keys(results.body?.counts || {}).length === 8);
